...
Smoker.prototype.pipe = function(stream) {
    ...
    this.targetHandler.pipeline(stream);
...
Smoker.prototype.process = function(basepath, opts, callback) {
    ...
    finder.on('end', function() {
        ...
        async.forEach(
            validFiles,
            smoker._processFile.bind(smoker, basepath, opts),
            callback
        );
        */
    });
...
Smoker.prototype._processFile = function(basepath, opts, targetFile, callback) {
    // run the filename through the matcher
    
    // check for handler matches
...
exports.createContext = function(buildfile) {
    ...
    smoke.out = function(relPath) {
        // initialise the basepath
        var basePath = path.join(path.dirname(buildfile), relPath || '');
        
        return function(file) {
            return fs.createWriteStream(path.join(basePath, file.name));
        };
    };
